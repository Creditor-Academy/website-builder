...
const FloatingImage = ({ component, section, isSelected, isEditing, editor, updateComponent, deleteComponent, selectComponent, selectSection }) => {
  const handleUpdate = (updates) => {
    if (updates.style) {
      updateComponent(section.id, component.id, { ...updates, style: { ...component.style, ...updates.style } });
    } else {
      updateComponent(section.id, component.id, updates);
    }
  };
  ...
  return (
    <TransformControls
      ...
      size={{ width: component.style?.width || '200px', height: component.style?.height || 'auto' }}
    ...
    >
      ...
      {isEditing && isSelected && !editor.previewMode && (
        <div
          className="comp-toolbar pointer-events-auto absolute -top-16 left-1/2 -translate-x-1/2 bg-white/95 backdrop-blur-md shadow-2xl rounded-2xl border border-slate-200 p-2.5 flex items-center gap-2 z-[70]"
        ...
        >
          <div className="flex items-center gap-1">
            <button
              onClick={() => handleUpdate({ style: { zIndex: (component.style?.zIndex || 10) + 10 } })}
            ...
            <button
              onClick={() => handleUpdate({ style: { zIndex: Math.max(0, (component.style?.zIndex || 10) - 10) } })}
              className="p-2 rounded-lg hover:bg-slate-50 text-slate-400 hover:text-slate-900 transition-colors"
              title="Send to Back"
            >
    ...
    >
      <div className={`w-full h-full ${!editor.previewMode && !isEditingText ? 'cursor-move' : ''}`}>
        {component.type === 'text' && (
          <div
            ref={contentRef}
            className={`w-full h-full outline-none min-w-[50px] whitespace-nowrap p-1 transition-all ${isEditingText ? 'cursor-text bg-white/50 backdrop-blur-sm rounded' : 'cursor-move'}`}
