...
export default function LoginSignup() {
  ...
  const LoginForm = () => (
    <div className="w-full max-w-[400px] flex flex-col justify-center h-full mx-auto">
      ...
      <button
        onClick={handleLogin}
        className="w-full bg-slate-950 hover:bg-slate-800 text-white font-medium py-4 rounded-xl transition-all active:scale-[0.98] mb-8"
      >
        Login
      </button>
  ...
  return (
    <div className="relative min-h-[100svh] w-full flex overflow-hidden bg-slate-950">
      ...
      {/* 2. LAYER TWO: ABSOLUTE NAV OVERLAYS ON TOP OF EVERYTHING */}
      ...
      {/* 3. LAYER THREE: MOBILE VIEW CENTERED BOX (Visible only md-) */}
      ...
      {/* 4. LAYER FOUR: DESKTOP FLOATING LAYOUT (Visible on md+) */}
      <div className="hidden md:block absolute inset-0 z-10 p-4">

        {/* ================= THE BACKGROUND PANELS WITH TEXT ================= */}

        {/* LEFT TEXT (Visible when white box is on the RIGHT) */}
        <AnimatePresence>
          {!isSignup && (
            <motion.div
        ...
        {/* RIGHT TEXT (Visible when white box is on the LEFT) */}
        <AnimatePresence>
          {isSignup && (
            <motion.div
              initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 50 }} transition={{ duration: 0.5, ease: "easeOut" }}
        ...
        {/* ================= THE FLOATING WHITE FORM CONTAINER ================= */}
        {/* This white card slides back and forth between the left side and right side of the screen */}
        <motion.div
          initial={false}
          animate={{
            /* 
               Calculate horizontal offset mapping directly to the reference:
               When on Login: Container sits on the RIGHT.
               When on Signup: Container sits on the LEFT.
               We have 1rem (16px) padding from parent `p-4`, so we move exactly 50vw + 1rem (or simple 100% of w-1/2).
            */
            x: isSignup ? "0%" : "calc(100% + 2rem)"
          }}
