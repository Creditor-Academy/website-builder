...
import { GripVertical, Eye, EyeOff, Copy, Trash2, Layout, Sparkles, Grid3X3, Image, MessageSquare, BarChart2, Users, HelpCircle, Building2, DollarSign, Mail, Quote, ArrowUp, ArrowDown } from 'lucide-react';
...
export function SectionItem({ id, name, type, visible, isSelected, onClick }) {
  const { toggleSectionVisibility, duplicateSection, deleteSection, moveSectionUp, moveSectionDown, state } = useBuilder();
  ...
  return (
    ...
    >
      ...
      <div className={`flex items-center gap-1 transition-all transform ${isSelected ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'} translate-x-1 group-hover:translate-x-0`}>
        <button
          onMouseDown={(e) => { console.log("Move Up button onMouseDown detected!"); e.stopPropagation(); }}
          onClick={(e) => { console.log("Move Up button clicked! ID:", id, "isFirst:", isFirst); moveSectionUp(id); }}
        ...
        <button
          onMouseDown={(e) => { console.log("Move Down button onMouseDown detected!"); e.stopPropagation(); }}
          onClick={(e) => { console.log("Move Down button clicked! ID:", id, "isLast:", isLast); moveSectionDown(id); }}
          className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-slate-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
